...
function checkdg() {
	...
	if (trim($el("addr2").value).length<3)  {
		$el("addr2").focus();
		alert("请详细填写您的街道地址（8个文字以上），以便我们方便联系您！");
		return false;
	}
		//获取产品信息
...
//		txtGust= $("#textareaGusta").text();
		...
		wfmon=198; 
		//获取联系时间段
		lxtime=$("#best_time option:selected").val();
		alert(lxtime);
		$.post("/index.php/Home/Index/order",{'checkb':checkb,'txtName':txtName,'txtMob':txtMob,'txtAddress':txtAddress,'hdmon':wfmon,'txtGust':lxtime},function(data){
			if(data=='奉请成功'){
                alert("奉请成功!");
